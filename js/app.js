  ...
  window.onload = function() {
      ...
      window.onscroll = function() {
        for (var i = 0; i < _fis.length; i++) {
          if (this.scrollY >= (getCoords(_fis[i]).top) * 0.35 && !_fis[i].classList.contains("anim-finished"))  {
        ...
        if (!sS_active) {
          ...
          if (this.scrollY >= _sSectionOffsetTop) {
        ...
        if (this.scrollY >= (_tSectionOffsetTop - _wHeight)) {
          if (!sS_active) {
            _sSection.children[0].style.top = "inherit";
            _sSection.children[0].style.bottom = 0;
          }
